...
const AddRecipe: React.FC = () => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>): void => {
    ...
    setRecipeData(prevData => ({
      ...prevData,
      [name]: name === 'dining_time' || name === 'preparation_time' || name === 'rest_time'
        ? value === '' ? null : value
        : value
    }));
  };
  ...
  const handleRemoveIngredient = (index: number) => {
    ...
    setRecipeData(prevData => ({
      ...prevData,
      ingredients: newIngredients.length > 0 ? newIngredients : [{ ingredient: '', amount: 0, unit: '' }],
    }));
  };

  const handleToolChange = (index: number, value: string) => {
    const newTools = recipeData.tools.map((tool, i) => i === index ? value : tool);
  ...
  const handleInstructionChange = (index: number, field: keyof Instruction, value: string) => {
    ...
    if (index === recipeData.instructions.length - 1 && value !== '') {
      newInstructions.push({ name: '', ingredients: [{ ingredient: '', amount: 0, unit: '' }], tools: [''], instruction: '' });
    }
  ...
  const handleRemoveInstruction = (index: number) => {
    ...
    setRecipeData(prevData => ({
      ...prevData,
      instructions: newInstructions.length > 0 ? newInstructions : [{ name: '', ingredients: [{ ingredient: '', amount: 0, unit: '' }], tools: [''], instruction: '' }],
    }));
  };

  const handleInstructionIngredientChange = (instructionIndex: number, ingredientIndex: number, field: keyof Ingredient, value: string | number) => {
    const newInstructions = recipeData.instructions.map((instruction, i) => {
      if (i === instructionIndex) {
        const newIngredients = instruction.ingredients.map((ingredient, j) =>
          j === ingredientIndex ? { ...ingredient, [field]: value } : ingredient
        );

        if (ingredientIndex === instruction.ingredients.length - 1 && value !== '') {
          newIngredients.push({ ingredient: '', amount: 0, unit: '' });
        }
  ...
  const handleRemoveInstructionIngredient = (instructionIndex: number, ingredientIndex: number) => {
    const newInstructions = recipeData.instructions.map((instruction, i) => {
      if (i === instructionIndex) {
        const newIngredients = instruction.ingredients.filter((_, j) => j !== ingredientIndex);
        return { ...instruction, ingredients: newIngredients.length > 0 ? newIngredients : [{ ingredient: '', amount: 0, unit: '' }] };
      }
      return instruction;
    });
  ...
  const handleInstructionToolChange = (instructionIndex: number, toolIndex: number, value: string) => {
    const newInstructions = recipeData.instructions.map((instruction, i) => {
      if (i === instructionIndex) {
  ...
  const handleRemoveInstructionTool = (instructionIndex: number, toolIndex: number) => {
    const newInstructions = recipeData.instructions.map((instruction, i) => {
      if (i === instructionIndex) {
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          {recipeData.instructions.map((instruction, index) => (
            <div key={index} className="space-y-2 mb-4 p-4 border border-gray-300 rounded-md shadow-sm">
              <input
                type="text"
              ...
              />
              <div>
                <label className="block text-sm font-medium text-gray-700">Instruction Ingredients:</label>
                {instruction.ingredients.map((ingredient, ingIndex) => (
                  <div key={ingIndex} className="flex space-x-2 mb-2">
                    <input
                      ...
                      placeholder="Ingredient"
                      value={ingredient.ingredient}
                      onChange={(e) => handleInstructionIngredientChange(index, ingIndex, 'ingredient', e.target.value)}
                      className="block w-1/3 border border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500"
                    />
                    <input
                      type="number"
                      placeholder="Amount"
                      value={ingredient.amount}
                      onChange={(e) => handleInstructionIngredientChange(index, ingIndex, 'amount', parseInt(e.target.value))}
                      className="block w-1/3 border border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500"
                    />
                    <input
                      type="text"
                      placeholder="Unit"
                      value={ingredient.unit}
                      onChange={(e) => handleInstructionIngredientChange(index, ingIndex, 'unit', e.target.value)}
                      className="block w-1/3 border border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500"
                    />
                    <button 
                      onClick={() => handleRemoveInstructionIngredient(index, ingIndex)} 
                      className="text-red-500"
                    >
                      x
                    </button>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700">Instruction Tools:</label>
                {instruction.tools.map((tool, toolIndex) => (
                  <div key={toolIndex} className="flex space-x-2 mb-2">
                    <input
                      type="text"
                      value={tool}
                      onChange={(e) => handleInstructionToolChange(index, toolIndex, e.target.value)}
                      className="block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500"
                    />
                    <button 
                      onClick={() => handleRemoveInstructionTool(index, toolIndex)} 
                      className="text-red-500"
                    >
                      x
                    </button>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">Recipe JSON:</label>
          <textarea
            rows={10}
            value={jsonInput}
