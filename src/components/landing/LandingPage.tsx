...
export default async function LandingPage() {
  ...
  return (
    <div className={`w-full overflow-x-clip ${lato.className}`}>
      <div className="absolute top-0 left-0 object-cover -z-30 w-full h-full">
        <Image
          src="/bg.webp"
          alt=""
          fill
          className="absolute object-cover w-full h-full top-0 left-0 -z-30 transform"
          priority
        />
            ...
            <div className="w-svw text-center md:text-start">
              {category.map(
                (recipes, index) =>
                  ...
                  recipes.length > 0 && (
                    <div
                      className="mb-8 mb:mt-12"
                      key={"recipe_slider_" + index}
                    >
                      <RecipeSlider
