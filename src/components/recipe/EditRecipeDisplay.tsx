...
const EditRecipeDisplay: React.FC<EditRecipeProps> = ({
  ...
  const handleLabelChange = (index: number, value: string) => {
    ...
    if (recipe.labels && index === recipe.labels.length - 1 && value !== "") {
      newLabels.push("");
    }
  ...
  const handleRemoveLabel = (index: number) => {
    ...
    const newRecipe = {
      ...recipe,
      labels: newLabels && newLabels.length > 0 ? newLabels : [""],
    };
    onRecipeChange(newRecipe);
    validateData(newRecipe);
  };
  ...
  const handleDiningTimeChange = (index: number, value: string) => {
    ...
    if (
      recipe.dining_times &&
      index === recipe.dining_times.length - 1 &&
      value !== ""
    ) {
      newDiningTimes.push("");
    }
  ...
  const handleRemoveDiningTime = (index: number) => {
    ...
    const newRecipe = {
      ...recipe,
      dining_times:
        newDiningTimes && newDiningTimes.length > 0 ? newDiningTimes : [""],
    };
    onRecipeChange(newRecipe);
    validateData(newRecipe);
  };
  ...
  const handleRemoveIngredient = (index: number) => {
    ...
    const newRecipe = {
      ...recipe,
      ingredients:
        newIngredients && newIngredients.length > 0
          ? newIngredients
  ...
  const handleToolChange = (index: number, value: string) => {
    ...
    if (recipe.tools && index === recipe.tools.length - 1 && value !== "") {
      newTools.push("");
    }
  ...
  const handleRemoveTool = (index: number) => {
    ...
    const newRecipe = {
      ...recipe,
      tools: newTools && newTools.length > 0 ? newTools : [""],
    };
    onRecipeChange(newRecipe);
    validateData(newRecipe);
  };
  ...
  const handleRemoveInstruction = (index: number) => {
    ...
    const newRecipe = {
      ...recipe,
      instructions:
        newInstructions && newInstructions.length > 0
          ? newInstructions
          : [
              {
                name: "",
  ...
  const handleInstructionIngredientChange = (
    ...
    const newIngredients = [
      ...(newInstructions[instructionIndex].ingredients || []),
    ];
